...
inline Matrix Matrix::Compose(const Vector3& position, const Quaternion& rotation, const Vector3& scale) {
	Matrix translation = XMMatrixTranslationFromVector(position);
	Matrix rotMatrix = XMMatrixRotationQuaternion(rotation);
	Matrix scaleMatrix = XMMatrixScalingFromVector(scale);
	return scaleMatrix * rotMatrix * translation;
}
...
inline Vector3 Quaternion::Euler() const {
	...
	v.y = std::atan2f(2.0f * q.x * q.w + 2.0f * q.y * q.z, 1.0f - 2.0f * (q.z * q.z + q.w * q.w));    // Yaw
	v.x = std::asinf(2.0f * (q.x * q.z - q.w * q.y));												// Pitch
	v.z = std::atan2f(2.0f * q.x * q.y + 2.0f * q.z * q.w, 1.0f - 2.0f * (q.y * q.y + q.z * q.z));    // Roll
	return NormalizeAngles(v);
}

inline Vector3 Quaternion::NormalizeAngles(Vector3 angles) {
...
inline float Quaternion::NormalizeAngle(float angle) {
	while (angle > 2.0f * PI)
		angle -= 2.0f * PI;
	while (angle < 0.0f)
		angle += 2.0f * PI;
...
inline Quaternion Quaternion::CreateFromEuler(const Vector3& euler) {
	using namespace DirectX;
	Quaternion R;
	XMStoreFloat4(&R, XMQuaternionRotationRollPitchYawFromVector(euler));
	return R;
}
