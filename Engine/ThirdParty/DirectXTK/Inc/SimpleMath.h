...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        struct Matrix : public XMFLOAT4X4
        {
			...
			Vector3 GetVector(int idx) const {
				switch (idx) {
				case 0:
					return Right();
				case 1:
			...
			Vector3 GetVectorNormalized(int idx) const {
				Vector3 temp;
				switch (idx) {
				case 0:
					temp = Right();
			...
			static Matrix Compose(const Vector3& position, const Quaternion& rotation, const Vector3& scale);
			...
			static Matrix fromString(std::string string);
