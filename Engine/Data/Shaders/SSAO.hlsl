...
Texture2D texWorldPos	: register (t1);
...
float PS_SSAO( VS_OUTPUT_POSTFX Input ) : SV_TARGET0
{
	float2 positionF = float2(Input.TextureUV * g_Viewport.xy);
	uint2 position = (uint2) positionF;

	float2 noiseScale = g_Viewport.xy / float2(8.0, 8.0);

	// World position
	//float3 WorldPos = texWorldPos.Load( uint3(position, 0) ).xyz;
	//float3 Normal = texNormals.Load( uint3(position, 0) ).xyz;
	float3 WorldPos = texWorldPos.Sample( samplerPointClamp, Input.TextureUV );

	float3 Normal = texNormals.Sample( samplerPointClamp, Input.TextureUV );
	Normal = normal_decode( Normal );
	Normal = normalize( mul(Normal, g_mtxView) );

	float3 randomVec = texNoise.Sample( samplerLinearWrap, Input.TextureUV * noiseScale ).xyz;
	...
	float userRadius = 1.5;
	float userBias = 0.025;

	float occlusion = 0.0;
	[unroll] for( int i=0; i < 32; i++)
	{
		// Get sample position
		float3 sampl = mul( samples[i], TBN );
		sampl = WorldPos + sampl * userRadius;

		// project sample position (to sample texture) (to get position on screen/texture)
		float4 offset = float4(sampl, 1.0);
		offset = mul(offset, g_mtxProjection );
		offset.xyz /= offset.w;
		offset.xyz = offset.xyz * 0.5 + 0.5;

		// get sample depth
		float sampleDepth = texWorldPos.Sample( samplerPointClamp, offset.xy ).z;	// get depth value of kernel sample

		// range check & accumulate
		float rangeCheck = smoothstep(0.0, 1.0, userRadius / abs( WorldPos.z - sampleDepth));
		occlusion += (sampleDepth >= sampl.z + userBias ? 1.0 : 0.0) * rangeCheck;
